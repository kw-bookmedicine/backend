...
public interface BoardRepository extends JpaRepository<Board,Long> {
    ...
    Page<Board> findByKeyword(Pageable pageable, @Param("keyword")Keyword keyword);

    //검색에 키워드까지 추가
    @Query("select b from Board b join fetch b.client c where (b.title like %:searchKeyword% or b.description like %:searchKeyword%)")
    Page<Board> findByTitleContainingOrDescriptionContaining(Pageable pageable,@Param("searchKeyword") String searchKeyword);

    @Query("select b from Board b join fetch b.client c where c.loginId = :loginId")
    Page<Board> findByUsername(Pageable pageable, @Param("loginId") String username);
