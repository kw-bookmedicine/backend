...
public interface ReadExperienceRepository extends JpaRepository<ReadExperience, Long> {
    ...
    @Query("select re from ReadExperience re join fetch re.client c join fetch re.book b" +
            " where c.loginId = :loginId and b.id = :bookId")
    Optional<ReadExperience> findByLoginIdAndBookId(@Param("loginId") String clientLoginId, @Param("bookId") Long bookId);

    /**
     * 책 id로 읽은 경험 조회하기
     * (이후에 있을 머신러닝에서 사용 : id로 이 책을 읽은 사람들 모두 조회 -> 그 사람들의 id로 독서경험을 모두 조회)
     */
    @EntityGraph(attributePaths = {"client"})
    @Query("select re from ReadExperience re join fetch re.book b where b.id = :bookId")
    List<ReadExperience> findByBookId(@Param("bookId") Long bookId);
